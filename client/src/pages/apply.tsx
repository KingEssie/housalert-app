      ...
      <main className="flex-1 max-w-xl mx-auto w-full pb-[120px] -mt-5 relative z-10 px-5 pt-5">
        <div className="space-y-4">
          <div className="app-card text-center">
            {listing.price > 0 && (
              <p className="text-[24px] font-bold text-[#000] leading-[1.1] tracking-[-0.02em]" data-testid="text-apply-price">
                €{listing.price}<span className="text-[14px] font-normal text-[#9CA3AF] ml-1">{t("common.perMonthShort")}</span>
              </p>
            )}
            <h1
              className="text-[20px] font-bold text-[#000] leading-[1.2] tracking-[-0.02em] mt-1.5"
              data-testid="text-apply-title"
            >
              {listing.title}
            </h1>
            <p className="text-[15px] text-[#9CA3AF] mt-1.5" data-testid="text-apply-subtitle">
              {subtitle}
            </p>
            {detailLine && (
              <p className="text-[13px] text-[#9CA3AF] mt-1" data-testid="text-apply-details">
                {detailLine}
              </p>
            )}
          </div>
          ...
          <div className="app-card">
            <h2 className="text-[16px] font-bold text-[#000] mb-1" data-testid="text-letter-title">{t("applySheet.applicationLetter")}</h2>
            <p className="text-[12px] text-[#9CA3AF] mb-3" data-testid="text-letter-helper">
              {t("applySheet.autoGenerated") || "Automatisch gegenereerd op basis van jouw profiel"}
            </p>
            <textarea
              className="w-full min-h-[220px] leading-[1.75] bg-white border-[2px] border-[#000] rounded-[6px] p-4 text-[15px] text-[#000] outline-none resize-vertical focus:border-ha-primary transition-colors"
