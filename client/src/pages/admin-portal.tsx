      ...
      <div>
        ...
        <div className="grid grid-cols-3 lg:grid-cols-6 gap-2">
          {[
            ...
            <button key={tab} onClick={() => onNavigate(tab)} className={`${CARD} p-3 flex flex-col items-center gap-1.5 hover:bg-[#F5F6F8] transition-colors`} data-testid={`quick-${tab}`}>
  ...
  return (
    <div className="space-y-4">
      ...
      {loading ? <LoadingState /> : (
        <div className={`${CARD} divide-y divide-[#F7F7F7]`}>
          {users.map(u => (
            <button key={u.user_id} onClick={() => openUser(u.user_id)} className="w-full px-4 py-3 flex items-center gap-3 text-left hover:bg-[#F5F6F8] transition-colors" data-testid={`user-card-${u.user_id}`}>
...
export default function AdminPortalPage() {
  ...
  if (checking) return <div className="min-h-screen bg-[#F5F6F8] flex items-center justify-center"><Loader2 className="w-7 h-7 text-ha-primary animate-spin" /></div>;
    ...
    <div className="min-h-screen bg-[#F5F6F8] flex">
      ...
      <aside className={`fixed lg:sticky top-0 left-0 z-50 h-screen w-[240px] bg-white border-r border-[#F0F0F0] flex flex-col transition-transform lg:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}>
          ...
          {TAB_CONFIG.map(({ id, label, icon: Icon }) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-[13px] font-medium mb-0.5 transition-colors ${active ? "bg-[#F7F7F7] text-[#111]" : "text-[#334855] hover:bg-[#F5F6F8] hover:text-[#111]"}`}
                data-testid={`nav-${id}`}
              >
        ...
        <div className="px-3 py-3 border-t border-[#F0F0F0]">
          <button
            onClick={() => navigate("/")}
            className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-[13px] font-medium text-[#334855] hover:bg-[#F5F6F8] hover:text-[#111] transition-colors"
