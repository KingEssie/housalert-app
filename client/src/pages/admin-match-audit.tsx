    ...
    <div className="min-h-screen bg-[#F7F7F7]" data-testid="page-admin-audit">
      ...
      <div className="max-w-xl mx-auto px-4 pb-32 space-y-5">
        <div className="bg-[#111111] rounded-2xl p-5 text-white" data-testid="card-account-info">
          <p className="text-[13px] text-[#9CA3AF] mb-1">Account</p>
          <p className="text-[16px] font-bold">{data.account.email}</p>
          <p className="text-[12px] text-[#9CA3AF] mt-1">ID: {data.account.user_id.substring(0, 8)}...</p>
          <div className="flex gap-4 mt-3 text-[13px]">
            <span className="text-[#9CA3AF]">Profiles: <strong className="text-white">{data.search_profiles.count}</strong></span>
            <span className="text-[#9CA3AF]">Sub: <strong className="text-white">{data.subscription?.status || "none"}</strong></span>
          </div>
        </div>

        <div className="grid grid-cols-2 gap-3">
          <StatCard icon={Database} label="Total Matches" value={stats.total} color="bg-ha-primary" />
          <StatCard icon={Eye} label="New (Unviewed)" value={stats.new_count} color="bg-[#6B7280]" />
              ...
              {recent_matches.map((m: any, i: number) => (
                <div key={m.id || i} className="py-3 border-b border-[#F7F7F7] last:border-0" data-testid={`match-row-${i}`}>
                  ...
                  <div className="flex items-center gap-3 mt-1.5 text-[11px]">
                    <span className={m.visible_in_app ? "text-green-600 font-medium" : "text-[#9CA3AF]"}>
                      {m.visible_in_app ? "✓ visible" : "✗ hidden"}
                    </span>
                    <span className={m.email_sent ? "text-ha-primary font-medium" : "text-[#9CA3AF]"}>
                      {m.email_sent ? "✓ emailed" : "✗ no email"}
                    </span>
                    <span className={m.push_sent ? "text-ha-primary font-medium" : "text-[#9CA3AF]"}>
                      {m.push_sent ? "✓ pushed" : "✗ no push"}
                    </span>
                    <span className={m.viewed ? "text-green-600" : "text-[#9CA3AF]"}>
                      {m.viewed ? "viewed" : "unviewed"}
                    </span>
                  </div>
                  {m.dedup_key && (
                    <p className="text-[10px] text-[#6B7280] mt-1 font-mono truncate">dedup: {m.dedup_key}</p>
                  )}
                </div>
              ))}
            </div>
