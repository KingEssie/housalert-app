...
function normalizeItem(item: DwListing, city: string): ParsedListing | null {
  ...
  return {
    ...
    size_m2: typeof item.groesse === "number" ? Math.round(item.groesse) : 0,
    bedrooms: typeof item.anzahl_zimmer === "number" ? Math.floor(item.anzahl_zimmer) : 0,
    rooms_decimal: typeof item.anzahl_zimmer === "number" && item.anzahl_zimmer > 0 ? item.anzahl_zimmer : null,
    image_url: imageUrl,
    district,
