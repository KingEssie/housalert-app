...
- `server/matching/engine.ts` — Central matching module with two main exports:
  ...
  - **Garden/Bath/Roof terrace**: Boolean filters via `extra_features[]`. 3-state: true/false/null. Extraction via shared `server/ingesters/feature-extraction.ts` with positive/negative German patterns. send_unclear controls null handling.
  ...
  - **Feature extraction**: Shared module `server/ingesters/feature-extraction.ts` — extractGarden, extractBath, extractRoofTerrace, extractEnergyLabel, extractPropertyTypeFromText, normalizePropertyType. All 3 ingesters use it.
