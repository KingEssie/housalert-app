...
- **Listing cards**: Reusable component in `client/src/components/listing-card.tsx` — three variants: `ListingCardFull` (matches/favorites tabs), `ListingCardCompact` (carousel), `ListingCardMini` (recently viewed). Full: edge-to-edge image (3:2 ratio), `rounded-[16px]` outer, 16px bold title, 15px bold price, 13px meta row with 14px icons, heart overlay on image. Compact: 16:9 image, `rounded-[16px]`. Mini: 1:1 image, `rounded-[14px]`. All white bg, no card shadow. Supports `locked` prop for subscription gate.
- **Listing detail**: `listing-detail.tsx` — white bg (no gray), flat content layout (no `app-card` wrappers), 4:3 hero image, `rounded-full` CTA button, `backdrop-blur-sm` bottom bar with price + action side by side. Back button: 40px `rounded-full` with `bg-white/90 backdrop-blur-sm`.
...
- `server/matching/engine.ts` — Central matching module with two main exports:
  ...
  - **Filter policy**: STRICT for core filters (city, price, bedrooms, size) — reject on mismatch. HYBRID for weak-metadata filters (furnished, pets_allowed, district, garden, bath, roof_terrace, energy_label, property_type, target_categories) — controlled by `send_unclear` toggle: ON allows null/unknown, OFF rejects null/unknown. Known mismatches always reject regardless of toggle.
  ...
  - **Property type matching**: Profile `property_types[]` compared against normalized `listing.property_type`. Canonical values: apartment, house, studio, room, shared, maisonette. Normalization maps German terms (Wohnung→apartment, Haus→house, etc.). Unknown type + send_unclear=ON → hybrid pass.
  - **Target categories matching**: Profile `target_categories[]` compared against listing `target_categories[]`. Overlap required. Empty listing categories treated as unknown → send_unclear controls.
  - **Garden/Bath/Roof terrace/Parking**: Boolean filters via `extra_features[]`. 3-state: true/false/null. Extraction via shared `server/ingesters/feature-extraction.ts` with positive/negative German patterns. send_unclear controls null handling. Parking extraction: Stellplatz, Parkplatz, Tiefgarage, Garage, PKW-Stellplatz, Carport, Duplex-Parker. DB column: `parking` boolean (migration 025).
  - **Energy label**: Extracted as text (A+, A, B, C, D, E, F, G, H). "energy_c" filter means "C or better" → ordered comparison (A+ < A < B < C passes, D/E/F/G rejects). null + send_unclear=ON → hybrid pass.
  - **Feature extraction**: Shared module `server/ingesters/feature-extraction.ts` — extractGarden, extractBath, extractRoofTerrace, extractParking, extractEnergyLabel, extractPropertyTypeFromText, normalizePropertyType. All 3 ingesters use it.
  - **Target categories population**: All 3 ingesters derive `target_categories` from `property_type` (e.g., if property_type="apartment", target_categories=["apartment"]). WG-Gesucht also maps its structured `category` field (0=room, 1=studio, 2=apartment, 3=house) as fallback when title-based extraction returns null.
  - **send_unclear toggle**: Controls ONLY unknown/null handling. Never overrides known mismatches. Default: ON (true). Stored as `search_profiles.send_unclear` boolean.
  - **price_flexible toggle**: Adds +10% buffer to price_max ceiling. Default: OFF (false). Stored as `search_profiles.price_flexible` boolean.
