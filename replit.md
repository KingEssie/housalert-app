...
- `server/matching/engine.ts` — Central matching module with two main exports:
  ...
  - **Pets filter** (HYBRID via extra_features): pets_allowed/huisdieren → listing.pets_allowed=true passes, null passes (hybrid), false rejects. Other extra_features (balcony, elevator) remain STRICT (null = rejected).
