...
- **Listing cards**: Reusable component in `client/src/components/listing-card.tsx` — three variants: `ListingCardFull` (matches/favorites tabs), `ListingCardCompact` (carousel), `ListingCardMini` (recently viewed). All share: image on top (16:10 ratio for full/compact, 1:1 for mini), white content section, `rounded-[--ha-card-radius] overflow-hidden bg-white shadow-ha-card`. Full variant: price badge on image, heart favorite, bottom feature row (city/bedrooms/m²), "X ago — website.com" meta line. Supports `locked` prop for subscription gate.
